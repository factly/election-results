#!/bin/bash
echo "********HELLO FOLKS, STARTING THE SCRIPT TO SCRAPE THE live RESULTS FROM ECIRESULTS************"
...
cd uttarakhand && scrapy crawl cw-all-candidates -a endpoint=live -t csv -o - > ../results/uttarakhand-cw-all-candidates.csv && cd ..
...
cd uttarakhand && scrapy crawl cw-trends -a endpoint=live -t csv -o - > ../results/uttarakhand-cw-trends.csv && cd ..
...
cd uttarpradesh && scrapy crawl cw-all-candidates -a endpoint=live -t csv -o - > ../results/uttarpradesh-cw-all-candidates.csv && cd ..
...
cd uttarpradesh && scrapy crawl cw-trends -a endpoint=live -t csv -o - > ../results/uttarpradesh-cw-trends.csv && cd ..
echo "********COMPLETED SCRAPING THE UTTAR PRADESH CONSTITUENCY-WISE TRENDS*************"

echo "********STARTING SCRAPING THE PARTY-WISE RESULTS***************"
cd pwresults && scrapy crawl pw-results -a endpoint=live -t csv -o - > ../results/pw-results.csv && cd .. 
