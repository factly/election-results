    ...
    def start_requests(self):
        for i in range(403):
            if self.endpoint == 'archive':            
                ...
                yield Request('http://eciresults.nic.in/ConstituencywiseS24%s.htm?ac=%s' % (i+1,i+1), callback=self.parse)              
...
class CWTrendsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        if self.endpoint == 'archive':
            ...
            yield Request('http://eciresults.nic.in/StatewiseS24.htm', callback=self.parse)
        for i in range(40):
            if self.endpoint == 'archive':
                yield Request('https://web.archive.org/web/20160823114553/http://eciresults.nic.in/StatewiseS03%s.htm' % (i+1), callback=self.parse)
            else:  
                yield Request('http://eciresults.nic.in/StatewiseS24%s.htm' % (i+1), callback=self.parse)
